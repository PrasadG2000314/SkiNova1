...
export default function DoctorAppointments() {
  ...
  const approvedAppointments = appointments.filter((apt) => {
    // ONLY show approved status AND must be future date
    if (apt.status !== "approved") {
      return false;
    }
    
    // Don't show if date has already passed
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 pt-40 pb-8 px-4 sm:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Manage all your patient appointments and requests</p>
        </div>
        ...
        {approvedAppointments.length > 0 && (
          <div className="rounded-3xl bg-white/20 backdrop-blur-xl border border-white/40 shadow-xl p-6 sm:p-8 mb-8">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                      ...
                      <div className="flex flex-col gap-2">
                        <button
                          onClick={() =>
                            router.push(
                              `/doctor/view-patient-reports?patientId=${apt.patientId}&appointmentId=${apt._id}&returnTo=/doctor/appointments`
                            )
                          }
                        ...
                        <button
                          onClick={() => router.push(`/chat?returnTo=/doctor/appointments`)}
            ...
            {approvedAppointments.length > 5 && (
              <div className="flex items-center justify-center gap-3 mt-6 pt-6 border-t border-white/20">
                ...
                <div className="flex gap-2">
                  {Array.from({ length: Math.ceil(approvedAppointments.length / 5) }).map((_, idx) => (
                    <button
                      key={idx + 1}
                ...
                <button
                  onClick={() => setCurrentPage((prev) => Math.min(prev + 1, Math.ceil(approvedAppointments.length / 5)))}
                  disabled={currentPage === Math.ceil(approvedAppointments.length / 5)}
                  className="p-2 rounded-xl bg-gradient-to-br from-white/40 to-white/20 text-gray-900 disabled:opacity-40 disabled:cursor-not-allowed hover:from-white/60 hover:to-white/40 transition-all duration-300 shadow-sm hover:shadow-md border border-white/30"
                >
                  <span className="text-lg">→</span>
                </button>
                ...
                <span className="ml-2 text-xs text-gray-700 font-medium bg-white/30 px-3 py-1.5 rounded-lg border border-white/30">
                  {currentPage} / {Math.ceil(approvedAppointments.length / 5)}
                </span>
              </div>
            )}
          </div>
